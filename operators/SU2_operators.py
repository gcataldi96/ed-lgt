...
def check_SU2_rishon_property(s):
    # Checks that the SU(2) rishon modes satisfies the SU2 algebra
    if not np.isscalar(s):
        raise TypeError(f"s must be SCALAR & (semi)INTEGER, not {type(s)}")
    # Define the Rishon operators
    ops = SU2_rishon_operators(s)
    # Difference between the left (and the right) Casimir Operators along the lattice link
    link_casimir = kron(ops["n"], ops["IDz"]) - kron(ops["IDz"], ops["n"])
    # Get the corresponding Kernel
    kernel = csr_matrix(null_space(link_casimir.toarray()))
    M = kernel * kernel.transpose()
    # Parallel Transporter
    U = {
        "up_up": kron((ops["za_up"] * ops["P_z"]), ops["zb_up_dag"])
        - kron((ops["zb_up"] * ops["P_z"]), ops["za_up_dag"]),
        "up_down": kron((ops["za_up"] * ops["P_z"]), ops["zb_down_dag"])
        - kron((ops["zb_up"] * ops["P_z"]), ops["za_down_dag"]),
        "down_up": kron((ops["za_down"] * ops["P_z"]), ops["zb_up_dag"])
        - kron((ops["zb_down"] * ops["P_z"]), ops["za_up_dag"]),
        "down_down": kron((ops["za_down"] * ops["P_z"]), ops["zb_down_dag"])
        - kron((ops["zb_down"] * ops["P_z"]), ops["za_down_dag"]),
    }
    # Perform the checks
    for op in U:
        norma = norm(comm(U[op], M))
        if norma > 1e-10:
            logger.info(s, op, norma)
            raise ValueError("The rishon operators are not well defined")
...
def check_SU2_rishon_algebra(s):
    ...
    # Define the rishon operators
    ops = SU2_rishon_operators(s)
    if check_matrix(2 * comm(ops["z_up"], T["Tx"]), ops["z_down"]):
        raise ValueError("ERROR")
    if check_matrix(2 * comm(ops["z_down"], T["Tx"]), ops["z_up"]):
        raise ValueError("ERROR")
    if check_matrix(2 * comm(ops["z_up"], T["Ty"]), -complex(0, 1) * ops["z_down"]):
        raise ValueError("ERROR")
    if check_matrix(2 * comm(ops["z_down"], T["Ty"]), complex(0, 1) * ops["z_up"]):
        raise ValueError("ERROR")
    if check_matrix(2 * comm(ops["z_up"], T["Tz"]), ops["z_up"]):
        raise ValueError("ERROR")
    if check_matrix(2 * comm(ops["z_down"], T["Tz"]), -ops["z_down"]):
        raise ValueError("ERROR")
    # CHECK RISHON MODES TO BEHAVE LIKE FERMIONS (anticommute with parity)
    for alpha in ["up", "down"]:
        if norm(anti_comm(ops[f"z_{alpha}"], ops["P_z"])) > 1e-15:
