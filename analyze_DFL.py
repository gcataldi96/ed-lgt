...
for case in ["noP", "P"]:
    config_filename = f"DFL/HSF_dyn_{case}"
    match = SimsQuery(group_glob=config_filename)
    ugrid, vals = uids_grid(match.uids, ["g", "m"])
    for obs in obs_list:
        res[case][obs] = np.zeros((len(vals["g"]), len(vals["m"])))
    for ii, g in enumerate(vals["g"]):
        for jj, m in enumerate(vals["m"]):
            for obs in obs_list:
                res[case][obs][ii, jj] = get_sim(ugrid[ii, jj]).res[obs]

for ii, g in enumerate(vals["g"]):
    for jj, m in enumerate(vals["m"]):
        for obs in obs_list:
            noP = res["noP"][obs][ii, jj]
            P = res["P"][obs][ii, jj]
            check = np.abs(noP - P)
            if check > 1e-3:
                print(g, m, obs, check, noP, P)
# %%
# HILBERT SPACE FRAGMENTATION vs QMB SCARS
local_obs = [f"T2_{s}{d}" for d in "x" for s in "mp"]
local_obs += ["E_square"]
local_obs += [f"N_{label}" for label in ["r", "g", "tot", "single", "pair"]]

res = {"HSF": {}, "QMBS": {}}
for ii, case in enumerate(["HSF", "QMBS"]):
    config_filename = f"DFL/{case}_dyn"
    match = SimsQuery(group_glob=config_filename)
    ugrid, vals = uids_grid(match.uids, ["g", "m"])
    res[case]["time"] = get_sim(ugrid[0, 0]).res["time_steps"]
    res[case]["F"] = get_sim(ugrid[0, 0]).res["overlap"]
    res[case]["entropy"] = get_sim(ugrid[0, 0]).res["entropy"]

fig, ax = plt.subplots(1, 1, constrained_layout=True)
ax.grid()
ax.set(
    ylabel=r"Fidelity $|\langle \psi(0)|\psi(t)\rangle|^{2}$",
    xlabel=r"time $t$",
    # xlim=[-0.1, 2]
)
...
np.savez_compressed(
    ...
    F_HSF=res["HSF"]["F"],
    F_QMBS=res["QMBS"]["F"],
    EE_HSF=res["HSF"]["entropy"],
    EE_QMBS=res["QMBS"]["entropy"],
)
fig, ax = plt.subplots(1, 1, constrained_layout=True)
ax.grid()
ax.set(
