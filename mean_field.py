...
def decomp_2body(C, m_A, n_A, m_B, n_B, error):
    """
    Input
...
def Ham_eff(ops :list, state):
    ...
    H_l,H_r=0,0
    H=0
    Id_A = np.identity(ops[0][0].shape[0])
    Id_B = np.identity(ops[0][1].shape[0])
    for Ai, Bi in ops:
...
def sim(ops,error_mean,error_dec):
    ...
    #test decomp
    test_decomp(ops_decomp,ops,atol=1e-13, rtol=1e-13)
