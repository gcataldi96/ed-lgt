...
res = {}
with np.load("wannier_TC_MM.npz", allow_pickle=False) as z:
    res["matrix"] = z["matrix"]
    res["kvals"] = z["kvals"]
    res["gs_energy"] = z["gs"]
# Given:
band_kernel = res["matrix"]
# band_kernel : (Nk, Nk) complex128   # M_{k1,k2} = <k1|H_0|k2>
# k_physical  : (Nk,) float64         # physical k in radians (spacing depends on T^2 vs TC)
Nk = band_kernel.shape[0]
k_physical = 2 * np.pi * res["kvals"] / Nk

R_target = 6  # choose where you want the Wannier localized (integer site)
weight_power = 1.0  # or 2.0 if you want to emphasize peaks
n_restarts = 10
seed = 0
maxiter = 5000
center_mode = 1
...
best_val = np.inf
best_theta = None
offset = np.complex128(res["gs_energy"])
...
@njit(cache=True)
def spread_functional(E_profile: np.ndarray, R_target: int) -> float:
    """
    Anchored MLWF spread around the chosen site R_target:
        σ^2(R_target) = (Σ_R w[R] * d(R,R_target)^2) / (Σ_R w[R]),
...
def objective(x):
    ...
    returns spread σ^2 around R_target for given phases
    """
    theta = np.empty(Nk, dtype=np.float64)
    # theta[0] = 0.0
    theta = x
    # 1) energy profile E_R(θ)
    E = energy_functional(band_kernel, k_physical, theta, offset)
    # 2) anchored spread around chosen site
    return spread_functional(E, R_target)
...
# Inspect the final energy profile
E_best = energy_functional(band_kernel, k_physical, best_theta, offset)
# %%
print("Optimal σ^2 estimated:", best_val)
print("Optimal θ phases (rad):", best_theta)
fig, ax = plt.subplots(1, 1, constrained_layout=True)
ax.plot(range(Nk), E_best, marker="o")  # - np.min(np.real(E_best)),
ax.set(xticks=np.arange(Nk))
ax.grid()
plt.ylim(1e-4, max(E_best) * 1.1)
# ax.set(yscale="log")
# %%
