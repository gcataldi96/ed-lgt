...
def Print_LOCAL_Observable(psi,nx,ny,Operator,staggered=False):
    ...
    print('----------------------------------------------------')
    print(f'    {Operator.Op_name} OPERATOR')
    print('----------------------------------------------------')
    ...
    for ii in range(n):
        ...
        exp_val[f'{Operator.Op_name}_{coords}']=np.dot(psi_dag,local_op(Operator.Op,Operator.ID,ii+1,n).dot(psi))
        # GET THE REAL PART
        exp_val[f'{Operator.Op_name}_{coords}']=np.real(exp_val[f'{Operator.Op_name}_{coords}'])
        # PRINT THE VALUE
        print(f'    {Operator.Op_name}_{coords}', format(exp_val[f'{Operator.Op_name}_{coords}'],'.12f'))
