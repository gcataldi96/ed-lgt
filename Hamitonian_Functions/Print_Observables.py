...
# =========================================================================================================
def Print_LOCAL_Observable(psi,nx,ny,Operator,staggered=False):
    # CHECK ON TYPES
    ...
    if not isinstance(Operator,Local_Operator):
       raise TypeError(f'Operator should be an instance of <class: Local_Operator>, not a {type(Operator)}')
    ...
    for ii in range(n):
        ...
        # Compute and print the HORIZONTAL LINK OPERATOR W
        exp_val[f'{Operator.Name}_{coords}']=np.dot(psi_dag,local_op(Operator.Op,Operator.ID,ii+1,n).dot(psi))
        # GET THE REAL PART
        exp_val[f'{Operator.Name}_{coords}']=np.real(exp_val[f'{Operator.Name}_{coords}'])
        # PRINT THE VALUE
        print(f'    {Operator.Name}_{coords}', format(exp_val[f'{Operator.Name}_{coords}'],'.12f'))
        ...
        if staggered:
            ...
            if staggered_factor<1:
               tmp_odd+=1
               odd_site_values[tmp_odd]=exp_val[f'{Operator.Name}_{coords}']
            else:
                tmp_even+=1
                even_site_values[tmp_even]=exp_val[f'{Operator.Name}_{coords}']
        else:
            values[ii]=exp_val[f'{Operator.Name}_{coords}']
    if staggered:
        return np.mean(odd_site_values), np.mean(even_site_values)
    else:
        return np.mean(values)


# =========================================================================================================
def Print_TWO_BODY_Correlators(exp_val,psi,nx,ny,Corr,periodicity=False):
    # CHECK ON TYPES
    if not isinstance(exp_val,dict):
        raise TypeError(f'exp_val should be a <dict>, not a {type(exp_val)}')
    if not isinstance(psi, np.ndarray):
        raise TypeError(f'psi should be an ndarray, not a {type(psi)}')
    if not np.isscalar(nx) and not isinstance(nx, int):
        raise TypeError(f'nx must be a SCALAR & INTEGER, not a {type(nx)}')
    ...
    for ii in range(n):
        ...
        if x<nx-1:
            ...
            h_coords='('+str(x+1)+','+str(y+1)+')----('+str(x+2)+','+str(y+1)+')'
            # Compute and print the HORIZONTAL 2BODY CORR
            exp_val[f'{Corr.Left_name}|{Corr.Right_name}_{h_coords}']=\
                np.real(np.dot(psi_dag,two_body_op(Op_HH_list,Corr.ID,Op_sites_list,n).dot(psi)))
            # PRINT THE VALUE
            print(f'    {Corr.Left_name}|{Corr.Right_name}_{h_coords}  ',\
                format(exp_val[f'{Corr.Left_name}|{Corr.Right_name}_{h_coords}'],'.12f'))
        else:
            if periodicity:
                Op_sites_list=[ii+1,ii-nx+2]
                # Define a string with the coords of the 2 points involved in the HORIZONTAL CORRELATOR
                h_coords='('+str(x+1)+','+str(y+1)+')----('+str(1)+','+str(y+1)+')'
                # Compute and print the HORIZONTAL 2BODY CORR
                exp_val[f'{Corr.Left_name}|{Corr.Right_name}_{h_coords}']=\
                    np.real(np.dot(psi_dag,two_body_op(Op_HH_list,Corr.ID,Op_sites_list,n).dot(psi)))
                # PRINT THE VALUE
        ...
        if y<ny-1:
            ...
            v_coords='('+str(x+1)+','+str(y+1)+')----('+str(x+1)+','+str(y+2)+')'
            # Compute and print the VERTICAL 2BODY CORR
            exp_val[f'{Corr.Bottom_name}|{Corr.Top_name}_{v_coords}']=\
                np.real(np.dot(psi_dag,two_body_op(Op_VV_list,Corr.ID,Op_sites_list,n).dot(psi)))
            # PRINT THE VALUE
            print(f'    {Corr.Bottom_name}|{Corr.Top_name}_{v_coords}  ',\
                format(exp_val[f'{Corr.Bottom_name}|{Corr.Top_name}_{v_coords}'],'.12f'))
        else:
            if periodicity:
                # Define a string with the coords of the 2 points involved in the VERTICAL CORRELATOR
                v_coords='('+str(x+1)+','+str(y+1)+')----('+str(x+1)+','+str(1)+')'
...
# =========================================================================================================
def Print_PLAQUETTE_Correlators(psi,nx,ny,Plaq,periodicity=False,not_Hermitian=False,get_real=False,get_imag=False):
    # CHECK ON TYPES
    if not isinstance(psi, np.ndarray):
        raise TypeError(f'psi should be an ndarray, not a {type(psi)}')
    if not np.isscalar(nx) and not isinstance(nx, int):
    ...
    if not_Hermitian==True:
        if get_real and not get_imag:
            ...
            raise ValueError('CANNOT HAVE get_real and get_imag CONCURRENTLY TRUE or CONCURRENTLY FALSE')
        # ADVERTISE OF THE CHOSEN PART OF THE PLAQUETTE YOU WANT TO COMPUTE
        print(f'      -------------')
        print(f'------| {chosen_part} PART |---------------------------------')
        print(f'      -------------')
    # Compute the total number of particles
    n=nx*ny
    # Compute the complex_conjugate of the ground state psi
    ...
    for ii in range(n):
        ...
        if x<nx-1 and y<ny-1:
            ...
            plaq_string='('+str(x+1)+','+str(y+1)+')__('+str(x+2)+','+str(y+1)+')__('\
                           +str(x+1)+','+str(y+2)+')__('+str(x+2)+','+str(y+2)+')'
            plaq_string=[str(x+1)+','+str(y+1),str(x+2)+','+str(y+1),str(x+1)+','+str(y+2),str(x+2)+','+str(y+2)]
            # COMPUTE THE PLAQUETTE
            exp_val[f'Plaq_{plaq_string}']=np.real(np.dot(psi_dag,\
            four_body_operator(Operator_list,Plaq.ID,Sites_list,n,get_only_part=chosen_part).dot(psi)))
            values[ii]=exp_val[f'Plaq_{plaq_string}']
            # PRINT THE PLAQUETTE
            #print(f'Plaq_{plaq_string}    ', format(exp_val[f'Plaq_{plaq_string}'],'.12f'))
            ...
            if periodicity:
                if x<nx-1 and y==ny-1:
                    ...
                    plaq_string='('+str(x+1)+','+str(y+1)+')__('+str(x+2)+','+str(y+1)+')__('\
                                   +str(x+1)+','+str(1)+')__('+str(x+2)+','+str(1)+')'
                    plaq_string=[str(x+1)+','+str(y+1),str(x+2)+','+str(y+1),str(x+1)+','+str(1),str(x+2)+','+str(1)]
                    # On the UPPER BORDER
                    jj=inverse_zig_zag(nx,ny,x,0)
                    # List of Sites where to apply Operators
                    ...
                    plaq_string='('+str(x+1)+','+str(y+1)+')__('+str(1)+','+str(y+1)+')__('\
                                   +str(x+1)+','+str(y+2)+')__('+str(1)+','+str(y+2)+')'
                    plaq_string=[str(x+1)+','+str(y+1),str(1)+','+str(y+1),str(x+1)+','+str(y+2),str(1)+','+str(y+2)]             
                    # On the RIGHT BORDER
                    # List of Sites where to apply Operators
                    ...
                    plaq_string='('+str(x+1)+','+str(y+1)+')__('+str(1)+','+str(y+1)+')__('\
                                   +str(x+1)+','+str(1)+')__('+str(1)+','+str(1)+')'
                    plaq_string=[str(x+1)+','+str(y+1),str(1)+','+str(y+1),str(x+1)+','+str(1),str(1)+','+str(1)]
                    # List of Sites where to apply Operators
...
# =========================================================================================================
def Print_BORDER_Penalties(exp_val,psi,nx,ny,Rishon):
    # CHECK ON TYPES
    if not isinstance(exp_val,dict):
        raise TypeError(f'exp_val should be a <dict>, not a {type(exp_val)}')
    if not isinstance(psi, np.ndarray):
    ...
    if not isinstance(Rishon,Rishon_Modes):
        raise TypeError(f'Rishon should be a CLASS <Rishon_Modes>, not a {type(Rishon)}')        
    # Compute the total number of particles
    n=nx*ny
    # Compute the complex_conjugate of the ground state psi
    psi_dag=np.conjugate(psi)
    for ii in range(n):
        ...
        if y==0:
            if x==0:
                ...
                exp_val[f'{Rishon.Left_name}_{coords}'] = np.dot(psi_dag,local_op(Rishon.Left,Rishon.ID,ii+1,n).dot(psi))
                exp_val[f'{Rishon.Left_name}_{coords}'] = np.real(exp_val[f'{Rishon.Left_name}_{coords}'])
                print(  f'{Rishon.Left_name}_{coords}    ',format(exp_val[f'{Rishon.Left_name}_{coords}'],'.12f'))
                #
                exp_val[f'{Rishon.Bottom_name}_{coords}'] = np.dot(psi_dag,local_op(Rishon.Bottom,Rishon.ID,ii+1,n).dot(psi))
                exp_val[f'{Rishon.Bottom_name}_{coords}'] = np.real(exp_val[f'{Rishon.Bottom_name}_{coords}'])
                print(  f'{Rishon.Bottom_name}_{coords}    ',format(exp_val[f'{Rishon.Bottom_name}_{coords}'],'.12f'))
            elif x==nx-1:
                # Compute and print RIGHT & BOTTOM Rishon modes
                exp_val[f'{Rishon.Right_name}_{coords}'] = np.dot(psi_dag,local_op(Rishon.Right,Rishon.ID,ii+1,n).dot(psi))
                exp_val[f'{Rishon.Right_name}_{coords}'] = np.real(exp_val[f'{Rishon.Right_name}_{coords}'])
                print(  f'{Rishon.Right_name}_{coords}    ',format(exp_val[f'{Rishon.Right_name}_{coords}'],'.12f'))
                #
                exp_val[f'{Rishon.Bottom_name}_{coords}'] = np.dot(psi_dag,local_op(Rishon.Bottom,Rishon.ID,ii+1,n).dot(psi))
                exp_val[f'{Rishon.Bottom_name}_{coords}'] = np.real(exp_val[f'{Rishon.Bottom_name}_{coords}'])
                print(  f'{Rishon.Bottom_name}_{coords}    ',format(exp_val[f'{Rishon.Bottom_name}_{coords}'],'.12f'))
            ...
            if x==0:
                ...
                print(  f'{Rishon.Left_name}_{coords}    ',format(exp_val[f'{Rishon.Left_name}_{coords}'],'.12f'))
                #
                exp_val[f'{Rishon.Top_name}_{coords}']=np.dot(psi_dag,local_op(Rishon.Top,Rishon.ID,ii+1,n).dot(psi))
                exp_val[f'{Rishon.Top_name}_{coords}'] = np.real(exp_val[f'{Rishon.Top_name}_{coords}'])
                print(  f'{Rishon.Top_name}_{coords}    ',format(exp_val[f'{Rishon.Top_name}_{coords}'],'.12f'))
            elif x==nx-1:
