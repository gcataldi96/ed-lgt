...
class Pure_State:
    def ground_state(self,Hamiltonian,debug_choice):
        self.energy, self.psi = get_ground_state_from_Hamiltonian(Hamiltonian,debug=debug_choice)
...
# ===================================================================
def get_reduced_density_matrix(psi,loc_dim,nx,ny,site,print_rho=False):
    """
    Parameters
    ----------
    psi : [type: ndarray]
    ...
    nx : [type: int]
        [number of sites on the x axis]
    ny : [type: int]
        [number of sites on the y axis]
    site : [type: int]
...
psi[5]=1/sqrt(3)
psi[7]=1/sqrt(3)
rho=sparse_density_matrix(psi)
n=3
loc_dim=2
#local_dimensions=np.array([loc_dim for jj in range(n)])
local_dimensions=np.array([2,2,2])
print(local_dimensions)
index=2
#rho_1=partial_trace(rho,local_dimensions,index)
