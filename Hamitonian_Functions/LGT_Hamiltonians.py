...
def Local_Hamiltonian(nx,ny,Operator,debug,staggered=False):
    ...
    if staggered:
        pause(f'    STAGGERED {Operator.Name} HAMILTONIAN',debug)
    else:
        pause(f'    {Operator.Name} HAMILTONIAN',debug)
    ...
    for ii in range(n):
        if staggered:
            ...
            print(f'    ({str(x+1)},{str(y+1)})  {str(staggered_factor)}')
...
def Two_Body_Hamiltonian(nx,ny,Corr,phrase,debug,periodicity=False,staggered=False,add_dagger=False,coeffs=None):
    ...
    if coeffs is None:
        ...
        if len(coeffs)!=2:
            raise ValueError(f'coeffs must be a LIST of 2 Coefficients, not of {str(len(coeffs))}')
    ...
    for ii in range(n):
        ...
        print(f'    ({str(x+1)},{str(y+1)})',coeffs)
        # ------------------------------------------------------------------------------
...
def Vocabulary_Local_Operator(nx,ny,Operator,debug,staggered=False):
    ...
    for ii in range(n):
        ...
        if staggered:
            # Compute the staggered factor depending on the site
            staggered_factor=(-1)**(x+y)
            vocabulary[f'{Operator.Name}_{point}']=\
                staggered_factor*local_op(Operator.Op,Operator.ID,ii+1,n)
            list_Names.append(f'{Operator.Name}_{point}')
        else:
            vocabulary[f'{Operator.Name}_{point}']=local_op(Operator.Op,Operator.ID,ii+1,n)
            list_Names.append(f'{Operator.Name}_{point}')
    return vocabulary, list_Names
...
def Vocabulary_Two_Body_Operator(nx,ny,Corr,Corr_name,debug,periodicity=False,staggered=False,add_dagger=False,coeffs=[1,1]):
    ...
    for ii in range(n):
        ...
        if x<nx-1:
            ...
            list_Names.append(f'{HH_Corr_Name}_{link}')
        else:
            if periodicity:
        ...
        if y<ny-1:
            ...
            list_Names.append(f'{VV_Corr_Name}_{link}')
        else:
            if periodicity:
                jj=inverse_zig_zag(nx,ny,x,0)
                Op_sites_list=[ii+1,jj+1]
...
def Vocabulary_Plaquette(nx,ny,Plaq,phrase,debug,periodicity=False,add_dagger=False):
    ...
    for ii in range(n):
        ...
        if x<nx-1 and y<ny-1:
            ...
            list_Names.append(f'Plaq_{point}')
        else:
            if periodicity:
                # PERIODIC BOUNDARY CONDITIONS
                if x<nx-1 and y==ny-1:
    ...
    for ii in range(n):
        ...
        if y==0:
            if x==0:
                ...
                list_Names.append(f'{Penalty.Left_name}_{Penalty.Bottom_name}_{point}')
            elif x==nx-1:
                # Kill RIGHT & BOTTOM Rishon modes
                vocabulary[f'{Penalty.Right_name}_{Penalty.Bottom_name}_{point}']=\
                    local_op(Penalty.Right*Penalty.Bottom,Penalty.ID,ii+1,n)
                list_Names.append(f'{Penalty.Right_name}_{Penalty.Bottom_name}_{point}')
            else:
                # Kill BOTTOM Rishon modes
                vocabulary[f'{Penalty.Bottom_name}_{point}']=\
                    local_op(Penalty.Bottom,Penalty.ID,ii+1,n)
                list_Names.append(f'{Penalty.Bottom_name}_{point}')
        elif y==ny-1:
            if x==0:
                # Kill LEFT & TOP Rishon modes
                vocabulary[f'{Penalty.Left_name}_{Penalty.TName}_{point}']=\
                    local_op(Penalty.Left*Penalty.Top,Penalty.ID,ii+1,n)
                list_Names.append(f'{Penalty.Left_name}_{Penalty.TName}_{point}')
            elif x==nx-1:
                # Kill RIGHT & TOP Rishon modes
                vocabulary[f'{Penalty.Right_name}_{Penalty.TName}_{point}']=\
                    local_op(Penalty.Right*Penalty.Top,Penalty.ID,ii+1,n)
                list_Names.append(f'{Penalty.Right_name}_{Penalty.TName}_{point}')
            else:
                # Kill TOP Rishon modes
                vocabulary[f'{Penalty.TName}_{point}']=\
                    local_op(Penalty.Top,Penalty.ID,ii+1,n)
                list_Names.append(f'{Penalty.TName}_{point}')
        else:
            if x==0:
                # Kill LEFT Rishon modes
                vocabulary[f'{Penalty.Left_name}_{point}']=\
                    local_op(Penalty.Left,Penalty.ID,ii+1,n)
                list_Names.append(f'{Penalty.Left_name}_{point}')
            elif x==nx-1:
                # Kill RIGHT Rishon modes
                vocabulary[f'{Penalty.Right_name}_{point}']=\
                    local_op(Penalty.Right,Penalty.ID,ii+1,n)
                list_Names.append(f'{Penalty.Right_name}_{point}')
    return vocabulary, list_Names