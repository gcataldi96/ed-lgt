...
def Local_Hamiltonian(nx, ny, Operator, debug, staggered=False):
    ...
    if not isinstance(Operator, Local_Operator):
        raise TypeError(
            f"Operator must be an  instance of <class: Local_Operator>, not a {type(Operator)}"
        )
    ...
    for ii in range(n):
        if staggered:
            ...
            H_Local = H_Local + staggered_factor * local_op(
                Operator.Op, Operator.ID, ii + 1, n
            )
        else:
...
def Two_Body_Hamiltonian(
    ...
    for ii in range(n):
        ...
        if x < nx - 1:
            ...
            Hamiltonian = Hamiltonian + coeffs[0] * two_body_op(
                Op_HH_list, Corr.ID, Op_sites_list, n
            )
            n_contributions += 1
        else:
            if periodicity == True:
        ...
        if y < ny - 1:
            ...
            Hamiltonian = Hamiltonian + coeffs[1] * two_body_op(
                Op_VV_list, Corr.ID, Op_sites_list, n
            )
            n_contributions += 1
        else:
            if periodicity == True:
...
def Plaquette_Hamiltonian(
    nx, ny, Plaq, phrase, debug, periodicity=False, add_dagger=False
):
    # CHECK ON TYPES
    if not np.isscalar(nx) and not isinstance(nx, int):
        raise TypeError(f"nx must be a SCALAR & INTEGER, not a {type(nx)}")
        ...
        if x < nx - 1 and y < ny - 1:
            ...
            if periodicity:
                ...
                if x < nx - 1 and y == ny - 1:
                    ...
                    H_Plaq = H_Plaq + four_body_operator(
                        Operator_list, Plaq.ID, Sites_list, n
                    )
                    n_plaquettes += 1
...
def Borders_Hamiltonian(nx, ny, Killed_Modes, phrase, debug):
    ...
    if not isinstance(Killed_Modes, Rishon_Modes):
        raise TypeError(
            f"Killed_Modes must be a CLASS <Penalties>, not a {type(Killed_Modes)}"
        )
    if not isinstance(phrase, str):
        raise TypeError(f"phrase must be a STRING, not a {type(phrase)}")
    if not isinstance(debug, bool):
    ...
    for ii in range(n):
        ...
        if y == 0:
            if x == 0:
                ...
                H_Borders = H_Borders + local_op(
                    Killed_Modes.Left * Killed_Modes.Bottom, Killed_Modes.ID, ii + 1, n
                )
                n_border_penalties += 1
            elif x == nx - 1:
                # Kill RIGHT & BOTTOM Rishon modes
                H_Borders = H_Borders + local_op(
                    Killed_Modes.Right * Killed_Modes.Bottom, Killed_Modes.ID, ii + 1, n
                )
                n_border_penalties += 1
            else:
                # Kill BOTTOM Rishon modes
                H_Borders = H_Borders + local_op(
                    Killed_Modes.Bottom, Killed_Modes.ID, ii + 1, n
                )
                n_border_penalties += 1
        elif y == ny - 1:
            if x == 0:
                ...
                H_Borders = H_Borders + local_op(
                    Killed_Modes.Left * Killed_Modes.Top, Killed_Modes.ID, ii + 1, n
                )
                n_border_penalties += 1
            elif x == nx - 1:
                # Kill RIGHT & TOP Rishon modes
                H_Borders = H_Borders + local_op(
                    Killed_Modes.Right * Killed_Modes.Top, Killed_Modes.ID, ii + 1, n
                )
                n_border_penalties += 1
            else:
                # Kill TOP Rishon modes
                H_Borders = H_Borders + local_op(
                    Killed_Modes.Top, Killed_Modes.ID, ii + 1, n
                )
                n_border_penalties += 1
        else:
            if x == 0:
                ...
                H_Borders = H_Borders + local_op(
                    Killed_Modes.Left, Killed_Modes.ID, ii + 1, n
                )
                n_border_penalties += 1
            elif x == nx - 1:
                # Kill RIGHT Rishon modes
                H_Borders = H_Borders + local_op(
                    Killed_Modes.Right, Killed_Modes.ID, ii + 1, n
                )
                n_border_penalties += 1
    if debug:
...
def Vocabulary_Local_Operator(nx, ny, Operator, debug, staggered=False):
    ...
    for ii in range(n):
        ...
        if staggered:
            # Compute the staggered factor depending on the site
            staggered_factor = (-1) ** (x + y)
            vocabulary[f"{Operator.Op_name}_{point}"] = staggered_factor * local_op(
                Operator.Op, Operator.ID, ii + 1, n
            )
            list_Op_names.append(f"{Operator.Op_name}_{point}")
        else:
            vocabulary[f"{Operator.Op_name}_{point}"] = local_op(
                Operator.Op, Operator.ID, ii + 1, n
            )
            list_Op_names.append(f"{Operator.Op_name}_{point}")
...
# ====================================================================
def Vocabulary_Two_Body_Operator(
    nx,
    ny,
    Corr,
    Corr_name,
    ...
    for ii in range(n):
        ...
        if x < nx - 1:
            ...
            vocabulary[f"{HH_Corr_Name}_{link}"] = coeffs[0] * two_body_op(
                Op_HH_list, Corr.ID, Op_sites_list, n
            )
        ...
        if y < ny - 1:
            ...
            x2, y2 = zig_zag(nx, ny, Op_sites_list[1] - 1)
            link = coords(x1, y1) + "__" + coords(x2, y2)
            vocabulary[f"{VV_Corr_Name}_{link}"] = coeffs[0] * two_body_op(
                Op_VV_list, Corr.ID, Op_sites_list, n
            )
...
def Vocabulary_Plaquette(
    ...
    for ii in range(n):
        ...
        if x < nx - 1 and y < ny - 1:
            # List of Sites where to apply Operators
            Sites_list = [ii + 1, ii + 2, ii + nx + 1, ii + nx + 2]
            vocabulary[f"Plaq_{point}"] = four_body_operator(
                Operator_list, Plaq.ID, Sites_list, n
            )
...
def Vocabulary_Penalty_Operators(nx, ny, Penalty, phrase, debug):
    ...
    if not isinstance(Penalty, Rishon_Modes):
        raise TypeError(
            f"Killed_Modes must be a CLASS <Penalties>, not a {type(Penalty)}"
        )
    if not isinstance(phrase, str):
        raise TypeError(f"phrase must be a STRING, not a {type(phrase)}")
    if not isinstance(debug, bool):
    ...
    for ii in range(n):
        ...
        if y == 0:
            if x == 0:
                # Kill LEFT & BOTTOM Rishon modes
                vocabulary[
                    f"{Penalty.Left_name}_{Penalty.Bottom_name}_{point}"
                ] = local_op(Penalty.Left * Penalty.Bottom, Penalty.ID, ii + 1, n)
                list_Op_names.append(
                    f"{Penalty.Left_name}_{Penalty.Bottom_name}_{point}"
                )
            elif x == nx - 1:
                # Kill RIGHT & BOTTOM Rishon modes
                vocabulary[
                    f"{Penalty.Right_name}_{Penalty.Bottom_name}_{point}"
                ...
                # Kill BOTTOM Rishon modes
                vocabulary[f"{Penalty.Bottom_name}_{point}"] = local_op(
                    Penalty.Bottom, Penalty.ID, ii + 1, n
                )
