...
class Local_Operator:
    def __init__(self, Operator, Operator_name):
        ...
        if not isinstance(Operator_name, str):
            raise TypeError(
                f"Operator_name should be a STRING, not a {type(Operator_name)}"
            )
        self.Op = Operator
    ...
    def get_identity(self, Identity):
        ...
        if not isspmatrix(Identity):
            raise TypeError(
                f"Identity should be a SPARSE MATRIX, not a {type(Identity)}"
            )
        self.ID = Identity
...
class Plaquette:
    def __init__(self, Op_Bottom_Left, Op_Bottom_Right, Op_Top_Left, Op_Top_Right):
        ...
        if not isspmatrix_csr(Op_Bottom_Left):
            raise TypeError(
                f"Op_Bottom_Left should be an CSR_MATRIX, not a {type(Op_Bottom_Left)}"
            )
        if not isspmatrix_csr(Op_Bottom_Right):
            raise TypeError(
                f"Op_Bottom_Right should be an CSR_MATRIX, not a {type(Op_Bottom_Right)}"
            )
        if not isspmatrix_csr(Op_Top_Left):
            raise TypeError(
                f"Op_Top_Left should be an CSR_MATRIX, not a {type(Op_Top_Left)}"
            )
        if not isspmatrix_csr(Op_Top_Right):
            raise TypeError(
                f"Op_Top_Right should be an CSR_MATRIX, not a {type(Op_Top_Right)}"
            )
    ...
    def add_Op_names(
        ...
        if not isinstance(Op_Bottom_Left_name, str):
            raise TypeError(
                f"Op_Bottom_Left_name should be a STRING, not a {type(Op_Bottom_Left_name)}"
            )
        if not isinstance(Op_Bottom_Right_name, str):
            raise TypeError(
                f"Op_Bottom_Right_name should be a STRING, not a {type(Op_Bottom_Right_name)}"
            )
        if not isinstance(Op_Top_Left_name, str):
            raise TypeError(
                f"Op_Top_Left_name should be a STRING, not a {type(Op_Top_Left_name)}"
            )
        if not isinstance(Op_Top_Right_name, str):
            raise TypeError(
                f"Op_Top_Right_name should be a STRING, not a {type(Op_Top_Right_name)}"
            )
...
class Rishon_Modes:
    ...
    def add_Op_names(self, P_Left_name, P_Right_name, P_Bottom_name, P_Top_name):
        if not isinstance(P_Left_name, str):
            raise TypeError(
                f"P_Left_name should be a STRING, not a {type(P_Left_name)}"
            )
        if not isinstance(P_Right_name, str):
            raise TypeError(
                f"P_Right_name should be a STRING, not a {type(P_Right_name)}"
            )
        if not isinstance(P_Bottom_name, str):
            raise TypeError(
                f"P_Bottom_name should be a STRING, not a {type(P_Bottom_name)}"
            )
...
class Two_Body_Correlator:
    def __init__(self, Op_Left, Op_Right, Op_Bottom, Op_Top):
        ...
        if not isspmatrix_csr(Op_Bottom):
            raise TypeError(
                f"Op_Bottom should be an CSR_MATRIX, not a {type(Op_Bottom)}"
            )
    ...
    def add_Op_names(self, Op_Left_name, Op_Right_name, Op_Bottom_name, Op_Top_name):
        if not isinstance(Op_Left_name, str):
            raise TypeError(
                f"Op_Left_name should be a STRING, not a {type(Op_Left_name)}"
            )
        if not isinstance(Op_Right_name, str):
            raise TypeError(
                f"Op_Right_name should be a STRING, not a {type(Op_Right_name)}"
            )
        if not isinstance(Op_Bottom_name, str):
            raise TypeError(
                f"Op_Bottom_name should be a STRING, not a {type(Op_Bottom_name)}"
            )
        if not isinstance(Op_Top_name, str):
            raise TypeError(
                f"Op_Top_name should be a STRING, not a {type(Op_Top_name)}"
            )
