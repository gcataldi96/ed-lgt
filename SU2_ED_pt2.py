...
H_Plaq,n_plaqs=Plaquette_Hamiltonian(nx,ny,SU2_Plaq,phrase,debug,\
        periodicity=PBC,add_dagger=True)
# COMPUTE THE LINK HAMILTONIAN
phrase='LINK-SYMMETRY HAMILTONIAN'
H_Link,n_link_penalties=Two_Body_Hamiltonian(nx,ny,SU2_W_Link,phrase,debug,periodicity=PBC)
...
if not pure_theory:
    ...
    H_Hopping, n_hopping=Two_Body_Hamiltonian(nx,ny,SU2_Hopping,phrase,debug,\
        periodicity=PBC,staggered=True,add_dagger=True,coeffs=[complex(0,-1.),1])
if not PBC:
...
for ii in set_parameters_list:
    ...
    print('----------------------------------------------------')
    print('')
    avg_plaquette.append(Print_PLAQUETTE_Correlators(GS.psi,nx,ny,SU2_Plaq,\
                periodicity=PBC,not_Hermitian=True,get_real=True,get_imag=False))
    print('')
    imag_plaquette=Print_PLAQUETTE_Correlators(GS.psi,nx,ny,SU2_Plaq,\
                periodicity=PBC,not_Hermitian=True,get_real=False,get_imag=True)
    if not pure_theory:
        # COMPUTE THE NUMBER DENSITY OPERATORS
        # N_SINGLE
        avg_single_odd, avg_single_even=Print_LOCAL_Observable(GS.psi,nx,ny,N_SINGLE,staggered=True)
        avg_N_SINGLE_ODD_site.append(avg_single_odd)
        avg_N_SINGLE_EVEN_site.append(avg_single_even)
        # N_PAIR
        avg_pair_odd, avg_pair_even=Print_LOCAL_Observable(GS.psi,nx,ny,N_PAIR,staggered=True)
        avg_N_PAIR_ODD_site.append(avg_pair_odd)
        avg_N_PAIR_EVEN_site.append(avg_pair_even)
        # N_TOT
        avg_tot_odd, avg_tot_even=Print_LOCAL_Observable(GS.psi,nx,ny,N_TOTAL,staggered=True)
    ...
    print('----------------------------------------------------')
    results=Print_TWO_BODY_Correlators(results,GS.psi,nx,ny,SU2_W_Link,periodicity=PBC)
    if PBC==False:
...
if len(set_parameters_list)>3:
    store_results(f'{RESULTS_directory}Simulation_{simulation_label}.txt',set_parameters_list,energy_set)
    store_results(f'{RESULTS_directory}Simulation_{simulation_label}.txt',set_parameters_list,avg_gamma)
    store_results(f'{RESULTS_directory}Simulation_{simulation_label}.txt',set_parameters_list,avg_plaquette)
    if not pure_theory:
        store_results(f'{RESULTS_directory}Simulation_{simulation_label}.txt',set_parameters_list,avg_N_SINGLE_EVEN_site)
        store_results(f'{RESULTS_directory}Simulation_{simulation_label}.txt',set_parameters_list,avg_N_SINGLE_ODD_site)
        store_results(f'{RESULTS_directory}Simulation_{simulation_label}.txt',set_parameters_list,avg_N_PAIR_EVEN_site)
